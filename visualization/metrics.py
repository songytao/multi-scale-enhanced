...
class HausdorffDistance:
    ...
    def compute(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        if torch.sum(pred) == 0:
            pred.fill_(1) if pred.numel() > 0 else pred.fill_(1).item()
            #pred[0][0][0].item() == 1
            # print(pred)
...
def evaluate(pred, gt):
    ...
    # # print('predicat_shape',pred.shape,gt.shape)
    # pred = pred.int()
...
class Metrics(object):
    ...
    def variance(self,name):
        # create a dictionary to store the variance of each metric
        var_metrics = {}
        # loop through the metrics dictionary
        for k, v in self.metrics.items():
            # calculate the variance of the metric list using numpy
            var_metrics[k] = np.var(v)
        # return the variance dictionary
        return var_metrics[name]
