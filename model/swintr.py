...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.strup = nn.ConvTranspose2d(self.embed_dim*8,self.embed_dim*4,2,stride=2)
    ...
    def GDFF(self,low_feature,high_feature):
        high_feature = nn.ConvTranspose2d(1024, 512, 2, stride=2)
    def swin_unet_decoder(self,swin_encoder_layer_x,x,c1,c2,c3,c4,c5):
        x = self._reshape_output(x)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.act1(x)  # x.shape = 7*7*768
        x = self.strup(x)  # x.shape = 14*14*384
        x = self.trconv0(x)  #  14**14*1024
    ...
    def forward(self, x):
        ...
        #print(x.shape)
        x = self.swin_unet_decoder(swin_encoder_layer_x,x,c1,c2,c3,c4,c5)

        return x


def swin_tiny_patch4_window7_224(num_classes: int = 1, **kwargs):
    # trained ImageNet-1K
